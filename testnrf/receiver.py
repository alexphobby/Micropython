...
def setup():
    ...
    while nrf == 0:
        try:
            nrf = NRF24L01(SPI(0), csn, ce, payload_size=4)
            write_custom_font.set_textpos(oled,0,0)
            write_custom_font.printstring("NRF OK ")
            oled.show()
...
while True:
    ...
    try:
        ...
        print("Sending i:{}".format(i))
        nrf.send(struct.pack("i", i))
        write_custom_font.set_textpos(oled,21,0)
        write_custom_font.printstring("s:" + str(i))
        oled.show()
        nrf.start_listening()
        buf = ""
        time.sleep(0.1)
        if nrf.any():
            ...
            while nrf.any():
                buf = nrf.recv()
                i_received = struct.unpack("ii", buf)
        
                print("sent, received:{}".format(i_received))
                write_custom_font.set_textpos(oled,42,0)
                write_custom_font.printstring(i_received)
                oled.show()
