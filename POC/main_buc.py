...
myStates = states.States(20,21,50,300,50,150,5000)
...
while not myStates.atSetpoint():

    if time.ticks_diff(tmp := time.ticks_ms(), last_run_time_dim1) >= int(myStates.timeStep1) and not myStates.atSetpoint1():
        last_run_time_dim1 = tmp
        myStates.setDimValueStep()
        #print(myStates.atSetpoint1())

    if time.ticks_diff(tmp := time.ticks_ms(), last_run_time_dim1) >= int(myStates.timeStep2) and not myStates.atSetpoint2():
        last_run_time_dim1 = tmp
        myStates.setDimValueStep()
