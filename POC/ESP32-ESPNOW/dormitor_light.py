...
async def read_pulse_in():
    ...
    while True:
        ...
        _first_pulse_in = machine.time_pulse_us(pwm_in,1,1000)
        _second_pulse_in = machine.time_pulse_us(pwm_in,1,10000)
        _third_pulse_in = machine.time_pulse_us(pwm_in,1,10000)
        _avg_pulse = int((_second_pulse_in + _third_pulse_in)/2)
        #print(f'{_second_pulse_in}/{_third_pulse_in} - avg:{_avg_pulse} old: {old_pulse_in} pulse_in: {pulse_in}')
        if abs(_avg_pulse - _second_pulse_in) > 5 or abs(_avg_pulse - old_pulse_in) < 5:
            continue
        
        #print(avg_pulse)
        old_pulse_in = _avg_pulse
...
async def print_pulse_in():
    ...
    while True:
        ...
        await dimmer.dimToPercent(pulse_in)
        #await asyncio.sleep(1)
