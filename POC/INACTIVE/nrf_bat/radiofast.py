...
class RadioFast(NRF24L01):
    ...
    def __init__(self, master, config):
        ...
        super().__init__(SPI(0,baudrate=10_000_000, polarity=0, phase=0,
            sck=Pin(2), mosi=Pin(3), miso=Pin(4)), Pin(config.csn_pin), Pin(config.ce_pin), config.channel, FromMaster.payload_size())
        if master:
