...
b_blue = Pin(21,Pin.IN,Pin.PULL_UP)
b_green = Pin(20,Pin.IN,Pin.PULL_UP)
throttle_val = 0
steering_val = 0

while True:
    print(f"{b_blue.value()};{b_green.value()}")
    time.sleep(0.3)
...
def master():
    ...
    while True:
        ...
        steering_val = int(steering.read_u16()/1000)
        ...
        if throttle_val != old_throttle_val or steering_val != old_steering_val:
            ...
            try:
                nrf.send(struct.pack("ii", throttle_val, steering_val))
                #print("sent")
            except OSError:
                pass
