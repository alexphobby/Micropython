...
btn_a = machine.Pin(16,Pin.IN,Pin.PULL_UP)
btn_b = machine.Pin(17,Pin.IN,Pin.PULL_UP)
btn_x = machine.Pin(18,Pin.IN,Pin.PULL_UP)
btn_y = machine.Pin(19,Pin.IN,Pin.PULL_UP)

steering_adj = 0

def adj_steering(pin,step):
    global steering_adj
    pin.irq(handler=None)
    steering_adj += step
    time.sleep_ms(100)
    pin.irq(handler=lambda p:adj_steering(p,step), trigger=Pin.IRQ_FALLING)

btn_x.irq(handler=lambda p:adj_steering(p,1),trigger=Pin.IRQ_FALLING)
btn_b.irq(handler=lambda p:adj_steering(p,-1),trigger=Pin.IRQ_FALLING)
...
def master():
    ...
    while True:
        ...
        #throttle_val = int(throttle.read_u16()/1000) 
        #steering_val = int(steering.read_u16()/1000) + steering_adj

        #if time.ticks_diff(tmp := time.ticks_ms(), last_sent) >= 100:
        #    last_sent = tmp
        #    timeout = True

            throttle_val = throttle.read_u16()
            steering_val = steering.read_u16() + steering_adj*1000

            if throttle_val != old_throttle_val or steering_val != old_steering_val:
                #write_custom_font.set_textpos(oled,0,0)
                #write_custom_font.printstring(f"{str(throttle_val)}     ")
                
                #write_custom_font.set_textpos(oled,30,0)
                #write_custom_font.printstring(f"{str(steering_val)}     ")
                
                #oled.show()
                #print(throttle_val,steering_val)
                try:
                    #(nrf.send_done() != None):
                    #    time.sleep_ms(10)
                    status = nrf.send_done()
                    #print(status)
                    nrf.send(struct.pack("ii", int(throttle_val/1000), int(steering_val/1000)))
                    #nrf.send(struct.pack("i",1))
                    old_throttle_val = throttle_val
                    old_steering_val = steering_val
                    #timeout = False
                    print(f"sent: {int(throttle_val/1000)}, {int(steering_val/1000)}")
                    time.sleep_ms(100)
                except OSError:
                    #print("err")
                    time.sleep_ms(10)
    ...
    while True:
        if nrf.any():
            ...
            while nrf.any():
                ...
                write_custom_font.set_textpos(oled,30,0)
                write_custom_font.printstring(led_state)
                oled.show()
